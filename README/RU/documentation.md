...
var ShareShow = (function(){
  ...
  var _setUpListners = function() {
      $('.share__btn_like').on('mouseenter', function(){
          $(this).stop(true,true);
          $('.share').addClass('open').animate({left: '0px' });   // Социальные иконки
      });
...
if(isPost()) {
  ...
  // Установление прозрачности изображения через альфа канал
  ...
  // Установление размеров изображения
  ...
  //Блокировка водяного знака Watermark block
  $isPattern = post('isPattern');
  if(bool($isPattern)) {
      // Смещение водяного знака от исходного изображения
      ...
      // Задание размеров исходного изображения
  ...
  // Отправление названия изоражения пользователю
...
// Получить ссылку на изображене и инициировать загрузку
$file_name = get('file');
send_file($file_name);
...
// Получить параметр Get get parameter
...
// Получить путь к файлу для его скачивания
function get_download_filedir($name) {
  global $download_dir;
  return ''.$download_dir.$name;
}
 
// Получить путь к файлу для его загрузки
...
// Инициализация загрузочного файла
